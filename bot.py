#!/usr/bin/env python3
"""
Telegram Video Downloader Bot with Compression (Railway-safe)
...
async def compress_video(input_path: Path, output_path: Path, target_res: str = "720p") -> Path:
    """Compress video with ffmpeg (lightweight for Railway)."""
    res_map = {
        "720p": "1280x720",
        "480p": "854x480",
        "360p": "640x360",
    ...
    def _compress():
        (
            ...
            .output(
                ...
                preset="ultrafast",   # very light on CPU
        ...
        try:
            ...
            if size > MAX_FILESIZE_BYTES:
                ...
                await context.bot.send_message(
                    chat_id=chat_id,
                    text=f"File is {round(size/1024/1024,2)} MB (>50 MB). Compress?",
                    reply_markup=kb
                )
                return
        ...
        try:
            ...
            with open(output_path, "rb") as f:
                await context.bot.send_video(
                    chat_id=chat_id,
                    video=f,
                    caption=f"Compressed to {resolution}, size {round(size/1024/1024,2)} MB"
                )
        finally:
            asyncio.create_task(delayed_cleanup(temp_dir, token))
